...
path <- "~/DPOT" ###\\\ CHANGE ME to the directory containing the required files.
source("DOPT_fun.R")	###\\\ CHANGE ME to the directory containing the required files.


#################
################# 1. Process DADA2 result
### import ASV taxonomy
ASV.taxonomy <- read.csv("dada2.ASV.taxonomy.csv")	###\\\ CHANGE file name or path if need
### import ASV fasta
ASV.seq <- readBStringSet("dada2.ASV.fasta")				###\\\ CHANGE file name or path if need
...
blast.subject <- "U16S/U16S.blast_20230118.blastdb"		 # CHANGE ME to the directory containing reference database with blast format DB
